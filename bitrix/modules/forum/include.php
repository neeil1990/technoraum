	...
	try
	{
		...
		if ($MESSAGE_TYPE == "EDIT")
		{
			...
			if (array_key_exists("TITLE", $arFieldsG))
			{
				...
				$fieldsBefore = $topic->getData();
				$result = $topic->edit($arFieldsG);
				if ($result->isSuccess())
				{
					$log = [];
					foreach ($fieldsBefore as $key => $val)
					{
						if ($fieldsBefore[$key] != $arFieldsG[$key])
						{
							$log[$key] =  $arFieldsG[$key];
							$log["before".$key] =  $fieldsBefore[$key];
						}
					}
					if (!empty($log))
					{
						CForumEventLog::Log("topic", "edit", $TID, serialize($log));
					}
				}
			}
			else
