...
//specific logic
define('BT_COND_LOGIC_PLUS', 8);                // +
define('BT_COND_LOGIC_MINUS', 9);                // -
define('BT_COND_LOGIC_MULTIPLY', 10);            // *
define('BT_COND_LOGIC_DIVIDE', 11);                // /
...
class AMGlobalCondCtrl{
    ...
    public static function GetLogic($arOperators = false){
        $arOperatorsList = array (
            ...
            BT_COND_LOGIC_PLUS => array (
                'ID'        => BT_COND_LOGIC_PLUS,
                'OP'        => array (
                    'N' => 'false === strpos(#FIELD#, #VALUE#)',
                    'Y' => 'CGlobalCondCtrl::LogicNotContain(#FIELD#, #VALUE#)'
                ),
                ...
                'MULTI_SEP' => ' && ',
                'VALUE'     => 'Plus',
                'LABEL'     => Loc::getMessage('BT_COND_LOGIC_PLUS_LABEL')
            ),
            BT_COND_LOGIC_MINUS => array (
                'ID'        => BT_COND_LOGIC_MINUS,
                'OP'        => array (
                    'N' => 'false === strpos(#FIELD#, #VALUE#)',
                    'Y' => 'CGlobalCondCtrl::LogicNotContain(#FIELD#, #VALUE#)'
                ),
                ...
                'MULTI_SEP' => ' && ',
                'VALUE'     => 'Minus',
                'LABEL'     => Loc::getMessage('BT_COND_LOGIC_MINUS_LABEL')
            ),
            BT_COND_LOGIC_MULTIPLY => array (
                'ID'        => BT_COND_LOGIC_MULTIPLY,
                'OP'        => array (
                    'N' => 'false === strpos(#FIELD#, #VALUE#)',
                    'Y' => 'CGlobalCondCtrl::LogicNotContain(#FIELD#, #VALUE#)'
                ),
                ...
                'MULTI_SEP' => ' && ',
                'VALUE'     => 'Multiply',
                'LABEL'     => Loc::getMessage('BT_COND_LOGIC_MULTIPLY_LABEL')
            ),
            BT_COND_LOGIC_DIVIDE => array (
                'ID'        => BT_COND_LOGIC_DIVIDE,
                'OP'        => array (
                    'N' => 'false === strpos(#FIELD#, #VALUE#)',
                    'Y' => 'CGlobalCondCtrl::LogicNotContain(#FIELD#, #VALUE#)'
                ),
                ...
                'MULTI_SEP' => ' && ',
                'VALUE'     => 'Divide',
                'LABEL'     => Loc::getMessage('BT_COND_LOGIC_DIVIDE_LABEL')
            )
        );
    ...
    public static function Check($arOneCondition, $arParams, $arControl, $boolShow){
        ...
        if( isset($arOneCondition['function']) ){
            $arValues['function'] = $arOneCondition['function'];
        }
            ...
            if ( !$boolValueError){
                ...
                switch ($arControl[ 'FIELD_TYPE' ]){
                    ...
                    case 'datetime':
                        ...
                        $boolValueError = AMGlobalCondCtrl::ConvertInt2DateTime($arOneCondition[ 'value' ], $strFormat, $intOffset);
                        break;
                    default:
                        $boolValueError = true;
                        break;
            ...
            if ( !$boolError){
                ...
                switch ($arControl[ 'FIELD_TYPE' ]){
                    ...
                    case 'datetime':
                        ...
                        $boolError = AMGlobalCondCtrl::ConvertDateTime2Int($arOneCondition[ 'value' ], $strFormat, $intOffset);
                        break;
                    default:
                        $boolError = true;
                        break;
                ...
                switch ($arControl[ 'PHP_VALUE' ][ 'VALIDATE' ]){
                    case 'element':
                        $rsItems = CIBlockElement::GetList(
                            array (), array ('ID' => $arOneCondition[ 'value' ]), false, false, array (
                                        'ID',
                                        'NAME'
                                    )
                        );
                    ...
                    case 'section':
                        $rsSections = CIBlockSection::GetList(
                            array (), array ('ID' => $arOneCondition[ 'value' ]), false, array (
                                        'ID',
                                        'NAME'
                                    )
                        );
                    ...
                    if ( !isset($arOneCondition[ $strID ])){
                        ...
                        switch ($arOneAtom[ 'ATOM' ][ 'FIELD_TYPE' ]){
                            ...
                            case 'datetime':
                                ...
                                $boolAtomError = AMGlobalCondCtrl::ConvertInt2DateTime($arOneCondition[ $strID ], $strFormat, $intOffset);
                                break;
                            default:
                                $boolAtomError = true;
                        }
                    ...
                    if ( !$boolAtomError){
                        switch ($arOneAtom[ 'ATOM' ][ 'FIELD_TYPE' ]){
                            ...
                            case 'datetime':
                                ...
                                $boolAtomError = AMGlobalCondCtrl::ConvertDateTime2Int($arOneCondition[ $strName ], $strFormat, $intOffset);
                                break;
                            default:
                                $boolAtomError = true;
                        }
            ...
            if ($boolShow){
                foreach ($arControl[ 'ATOMS' ] as &$arOneAtom){
                    ...
                    switch ($arOneAtom[ 'ATOM' ][ 'VALIDATE' ]){
                        ...
                        case 'element':
                            $rsItems = CIBlockElement::GetList(
                                array (), array ('ID' => $arValues[ $strID ]), false, false, array (
                                'ID',
                                'NAME'
                            )
                            );
                        ...
                        case 'section':
                            $rsSections = CIBlockSection::GetList(
                                array (), array ('ID' => $arValues[ $strID ]), false, array (
                                'ID',
                                'NAME'
                            )
                            );
...
class AMGlobalCondCtrlComplex extends AMGlobalCondCtrl{
    public static function GetControlDescr(){
        $strClassName = get_called_class();
                    ...
                    case 'text':
                        if (is_array($arValues[ 'value' ])){
                            ...
                            $strResult = str_replace(
                                array ('#FIELD#', '#VALUE#'), array (
                                                                $strField,
                                                                '"' . EscapePHPString($arValues[ 'value' ]) . '"'
                                                            ), $arLogic[ 'OP' ][ $arControl[ 'MULTIPLE' ] ]
                            );
                        }
                        break;
...
class AMGlobalCondCtrlGroup extends AMGlobalCondCtrl{
    public static function GetControlDescr(){
        $strClassName = get_called_class();
    ...
    public static function GetConditionShow($arParams){
        ...
        foreach ($arAtoms as &$arOneAtom){
            if ( !isset($arParams[ 'DATA' ][ $arOneAtom[ 'id' ] ]) || !is_string($arParams[ 'DATA' ][ $arOneAtom[ 'id' ] ]) || (isset($arOneAtom[ 'values' ]) && !isset($arOneAtom[ 'values' ][ $arParams[ 'DATA' ][ $arOneAtom[ 'id' ] ] ]))
            ){
                $boolError = true;
            }
            if ( !$boolError){
    ...
    public static function Parse($arOneCondition){
        ...
        $arAtoms = static::GetAtoms();
        foreach ($arAtoms as &$arOneAtom){
            if ( !isset($arOneCondition[ $arOneAtom[ 'name' ] ]) || !is_string($arOneCondition[ $arOneAtom[ 'name' ] ]) || !isset($arOneAtom[ 'values' ][ $arOneCondition[ $arOneAtom[ 'name' ] ] ])
            ){
                $boolError = true;
            }
            if ( !$boolError){
    ...
    public static function Generate($arOneCondition, $arParams, $arControl, $arSubs = false){
        ...
        foreach ($arAtoms as &$arOneAtom){
            if ( !isset($arOneCondition[ $arOneAtom[ 'id' ] ]) || !is_string($arOneCondition[ $arOneAtom[ 'id' ] ]) || !isset($arOneAtom[ 'values' ][ $arOneCondition[ $arOneAtom[ 'id' ] ] ])
            ){
                $boolError = true;
            }
        }
...
class AMCatalogCondCtrl extends AMGlobalCondCtrl{
}

class AMCatalogCondCtrlComplex extends AMGlobalCondCtrlComplex{
}

class AMCatalogCondCtrlGroup extends AMGlobalCondCtrlGroup{
}

class AMCatalogCondCtrlIBlockFields extends AMCatalogCondCtrlComplex{
    /**
     * @return string|array
     */
    public static function GetControlID(){
    ...
    public static function GetControls($strControlID = false){
        ...
        $arControlList = array (
            ...
            'CondIBSectionWithSubsection'        => array (
                'ID'         => 'CondIBSectionWithSubsection',
                'PARENT'     => false,
                'FIELD'      => 'SECTION_ID',
                'FIELD_TYPE' => 'int',
                'LABEL'      => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_IBLOCK_SECTION_WTH_SUB_ID_LABEL'),
                'PREFIX'     => Loc::getMessage('BT_MOD_CATALOG_COND_CMP_IBLOCK_SECTION_WTH_SUB_ID_PREFIX'),
                'LOGIC'      => static::GetLogic(
                    array (
                        BT_COND_LOGIC_EQ,
                        BT_COND_LOGIC_NOT_EQ
        ...
        $arControlList[ 'CondIBSectionWithSubsection' ][ 'MULTIPLE' ] = 'Y';
...
class AMCatalogCondCtrlIBlockProps extends AMCatalogCondCtrlComplex{
    /**
     * @param bool|string $strControlID
     *
     * @return bool|array
    ...
    public function OnConditionControlBuildList(){
        if ( !$this->boolError && !isset($this->arControlList)){
            ...
            foreach (GetModuleEvents($this->arEvents[ 'CONTROLS' ][ 'MODULE_ID' ], $this->arEvents[ 'CONTROLS' ][ 'EVENT_ID' ], true) as $arEvent){
                ...
                if (isset($arRes[ 'ID' ])){
                    ...
                    if (isset($this->arControlList[ $arRes[ 'ID' ] ])){
                        $this->arMsg[ ] = array (
                            'id'   => 'CONTROLS',
                            'text' => str_replace('#CONTROL#', $arRes[ 'ID' ], Loc::getMessage('BT_MOD_COND_ERR_CONTROL_DOUBLE'))
                        );
                        $this->boolError = true;
                    }else{
                        ...
                        foreach ($arRes[ 'CONTROLS' ] as &$arOneControl){
                            if (isset($arOneControl[ 'ID' ])){
                                ...
                                if (isset($this->arControlList[ $control[ 'ID' ] ])){
                                    $this->arMsg[ ] = array (
                                        'id'   => 'CONTROLS',
                                        'text' => str_replace('#CONTROL#', $control[ 'ID' ], Loc::getMessage('BT_MOD_COND_ERR_CONTROL_DOUBLE'))
                                    );
                                    $this->boolError = true;
                                }else{
                    ...
                    foreach ($arRes as &$arOneRes){
                        if (is_array($arOneRes) && isset($arOneRes[ 'ID' ])){
                            ...
                            if (isset($this->arControlList[ $arOneRes[ 'ID' ] ])){
                                $this->arMsg[ ] = array (
                                    'id'   => 'CONTROLS',
                                    'text' => str_replace('#CONTROL#', $arOneRes[ 'ID' ], Loc::getMessage('BT_MOD_COND_ERR_CONTROL_DOUBLE'))
                                );
                                $this->boolError = true;
                            }else{
            ...
            if (empty($this->arControlList)){
                $this->arMsg[ ] = array (
                    'id'   => 'CONTROLS',
                    'text' => Loc::getMessage('BT_MOD_COND_ERR_CONTROLS_EMPTY')
                );
                $this->boolError = true;
            }
        }
    ...
    protected function GetEventList($intEventID){
        $arEventList = array (
            BT_COND_BUILD_CATALOG      => array (
                ...
                'CONTROLS' => array (
                    'MODULE_ID' => 'dwstroy.pricechanger',
                    'EVENT_ID'  => 'OnCondCatControlBuildListAM'
                )
            ),
            ...
            BT_COND_BUILD_SALE_ACTIONS => array (
                ...
                'CONTROLS' => array (
                    'MODULE_ID' => 'sale',
                    'EVENT_ID'  => 'OnCondSaleActionsControlBuildListAM'
                )
            )
        );
    ...
    public function Init($intMode, $mxEvent, $arParams = array ()){
        ...
        if (is_array($mxEvent)){
            if (isset($mxEvent[ 'CONTROLS' ]) && $this->CheckEvent($mxEvent[ 'CONTROLS' ])){
                ...
                $this->arMsg[ ] = array (
                    'id'   => 'EVENT',
                    'text' => Loc::getMessage('BT_MOD_COND_ERR_EVENT_BAD')
                );
            }
        ...
        if (BT_COND_MODE_DEFAULT == $this->intMode){
            ...
            if (empty($this->strJSName)){
                if (empty($this->strContID)){
                    ...
                    $this->arMsg[ ] = array (
                        'id'   => 'JS_NAME',
                        'text' => Loc::getMessage('BT_MOD_COND_ERR_JS_NAME_BAD')
                    );
                }else{
        ...
        if ( !$this->boolError){
            foreach ($arData as $strKey => $value){
                ...
                $arOneCondition = call_user_func_array(
                    $this->arControlList[ $value[ 'controlId' ] ][ 'Parse' ], array (
                                                                                $value
                                                                            )
                );
                ...
                if( isset($value['function']) ){
                    $arOneCondition['function'] = $value['function'];
                }
            ...
            if (isset($arLevel[ 'CLASS_ID' ]) && !empty($arLevel[ 'CLASS_ID' ])){
                if (isset($this->arControlList[ $arLevel[ 'CLASS_ID' ] ])){
                    ...
                    $arOneResult = call_user_func_array(
                        $arOneControl[ "GetConditionShow" ], array (
                                                               $arParams,
                                                           )
                    );
                    ...
                    if (isset($this->arControlList[ $arLevel[ 'CLASS_ID' ] ])){
                        ...
                        if ('Y' == $arOneControl[ 'GROUP' ]){
                            if (call_user_func_array(
                                $arOneControl[ 'ApplyValues' ], array (
                                                                  $arLevel[ 'DATA' ],
                                                                  $arLevel[ 'CLASS_ID' ]
                                                              )
                            )){
...
class AMCondTree extends AMGlobalCondTree{
    public function __construct(){
