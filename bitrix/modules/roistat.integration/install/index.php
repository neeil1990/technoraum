...
Class roistat_integration extends CModule
{
    ...
    function InstallDB($arParams = array())
    {
        ...
        if ($this->errors !== false) {
            ...
            if (COption::GetOptionString('sale', 'expiration_processing_events') === 'Y') {
                if (count(GetModuleEvents('sale', 'OnOrderSaved', true)) !== 0) {
                    RegisterModuleDependences('sale', 'OnOrderSaved', 'roistat.integration', 'CRoistat', 'OnOrderSaveHandler', 10001);
                }
                RegisterModuleDependences('sale', 'OnOrderSave', 'roistat.integration', 'CRoistat', 'OnOrderSaveHandler', 10001);
                RegisterModuleDependences('sale', 'OnOrderNewSendEmail', 'roistat.integration', 'CRoistat', 'OnOrderNewSendEmailHandler');
            } else {
                RegisterModuleDependences('sale', 'OnSaleOrderSaved', 'roistat.integration', 'CRoistat', 'OnSaleOrderSavedHandler', 10001);
            }
        }
    ...
    function UnInstallDB($arParams = array())
    {
        ...
        if (COption::GetOptionString('sale', 'expiration_processing_events') === 'Y') {
            ...
            UnRegisterModuleDependences('sale', 'OnOrderSave', 'roistat.integration', 'CRoistat', 'OnOrderSaveHandler');
            if (count(GetModuleEvents('sale', 'OnOrderSaved', true)) !== 0) {
                UnRegisterModuleDependences('sale', 'OnOrderSaved', 'roistat.integration', 'CRoistat', 'OnOrderSaveHandler');
            }
        } else {
            UnRegisterModuleDependences('sale', 'OnSaleOrderSaved', 'roistat.integration', 'CRoistat', 'OnSaleOrderSavedHandler');
        }
        UnRegisterModuleDependences('main', 'OnEndBufferContent', 'roistat.integration', 'CRoistat', 'OnEndBufferContentHandler');
