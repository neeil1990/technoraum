...
define('LOAD_FILE', 'GeoLite2-City.tar.gz');
...
$sSmUpDir = "/upload/altasib/geobase/";
$sUploadDir = $_SERVER["DOCUMENT_ROOT"] . $sSmUpDir;
$sNewDir = "GeoLite2";
...
if ($strAction == "UPDATE"){
	if (CAltasibGeoBase::GetIsUpdateDataFile(LOAD_HOST, LOAD_PATH, LOAD_FILE, $strFilename)){
		CAdminNotify::Add(
			array(
				"MESSAGE"		=> GetMessage("ALTASIB_GEOBASE_MM_GEOLITE2_THERE_IS"),
...
elseif ($strAction == "UNPACK"){
	...
	if($tRes) {
		if (!$oArchiver->bFinish){
			...
			$response["FILENAME"]	= urlencode(basename($sNewDir));
			$response["DROP_T"]		= "Y";

			// Delete old directory
			if(file_exists($sUploadDir . $sNewDir) && is_dir($sUploadDir . $sNewDir)){
				DeleteDirFilesEx($sSmUpDir . $sNewDir);
			}
			// Search and rename the subdirectory, since it contains the generation date
			$scanRes = scandir($sUploadDir);
			foreach($scanRes as $dirVal) {
				if(!in_array($dirVal, array('.', '..'))){
					if(is_dir($sUploadDir . $dirVal)){
						if(stripos($dirVal, "GeoLite2-City")!==false){
							rename($sUploadDir . $dirVal, $sUploadDir . $sNewDir);
						}
					}
				}
			}
