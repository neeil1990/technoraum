...
class Temporary extends Base
{
	...
	protected $elementTableName;
	protected $offerTableName;
	...
	protected function insertElements($filter, $context)
	{
		$connection = $this->getConnection();
		$sqlHelper = $connection->getSqlHelper();
		$tableName = $this->getElementTableName();
	...
	protected function insertOffers($filter, $context)
	{
		$connection = $this->getConnection();
		$sqlHelper = $connection->getSqlHelper();
		...
		$elementSql = $this->getElementSql($filter, $select);
		$propertyOriginalField = $this->getSqlOriginalField($elementSql, 'PROPERTY_' . $context['OFFER_PROPERTY_ID'] . '_VALUE');
		$propertyFilter = $propertyOriginalField . ' IN ('
			. 'SELECT ' . $sqlHelper->quote('ID')
			. ' FROM ' . $sqlHelper->quote($elementTableName)
			. ' WHERE ' . $sqlHelper->quote('ITERATION') . ' = ' . $this->iteration
		. ')';
		$elementSql = preg_replace('/(\sWHERE\s.*)(\sGROUP BY\s.*)?$/si', '$1 AND ' . $propertyFilter . ' $2', $elementSql);
