...
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/vsfr.merchant/prolog.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/vsfr.merchant/mysql/vsfr_catalog_export.php");
...
CModule::IncludeModule("vsfr.merchant");
...
$tp = getPHPExecutableFromPath();

$arReportsList = GetReportsList("/bitrix/php_interface/include/vsfr.merchant/");
		...
		if (strlen($strErrorMessage)<=0)
		{
			...
			if ($_REQUEST["ACTION"]=="EXPORT" && $bCanExec)
			{
				...
				else
				{
					...
					if ($PROFILE_ID <= 0)
					{
						$db_profile = CCatalogVSFRExport::GetList(
							array(),
...
setTimeout('DoNext()', 2000);
				...
				{
					$PROFILE_ID = CCatalogVSFRExport::Add(
						array(
							"=LAST_USE" => $DB->GetNowFunction(),
				...
				{
					$strSetupFileName = '';
					$strRedirect = '/bitrix/admin/vsfr_export_setup.php?lang='.urlencode(LANGUAGE_ID).'&success_export=Y';
			...
			elseif ($_REQUEST["ACTION"]=="MENU" && $bCanEdit)
			{
				...
				if ($PROFILE_ID <= 0)
				{
					$db_profile = CCatalogVSFRExport::GetList(array(), array("DEFAULT_PROFILE"=>"Y", "FILE_NAME"=>$strActFileName));
					if ($ar_profile = $db_profile->Fetch())
						$PROFILE_ID = (int)$ar_profile['ID'];
				}
				...
				{
					$PROFILE_ID = CCatalogVSFRExport::Add(array(
						"LAST_USE"		=> false,
						"FILE_NAME"		=> $strActFileName,
						"NAME"			=> $arReportsList[$strActFileName]["TITLE"],
						"DEFAULT_PROFILE" => "Y",
				...
				if (strlen($strErrorMessage)<=0)
				{
					...
					if ($PROFILE_ID>0)
					{
						if ($ar_profile["IN_AGENT"]=="Y")
							CAgent::RemoveAgent("CCatalogVSFRExport::PreGenerateExport(".$PROFILE_ID.");", "catalog");
						else
							CAgent::AddAgent("CCatalogVSFRExport::PreGenerateExport(".$PROFILE_ID.");", "catalog", "N", $agent_period*60*60, "", "Y");
			...
			elseif ($_REQUEST["ACTION"]=="CRON" && $USER->CanDoOperation('edit_php'))
			{
				...
				if (strlen($strErrorMessage)<=0)
				{
					...
					if (strlen($agent_php_path)<=0) $agent_php_path = $tp;
					
					if (strlen($agent_php_path)<=0) $agent_php_path = "/usr/local/php/bin/php";
					

					if (!file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/vsfr.merchant/cron_frame.php"))
					{
						CheckDirPath($_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/vsfr.merchant/");
						$tmp_file_size = filesize($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/vsfr.merchant/load/cron_frame.php");
						$fp = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/vsfr.merchant/load/cron_frame.php", "rb");
						...
						$fp = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/vsfr.merchant/cron_frame.php", "ab");
						fwrite($fp, $tmp_data);
						fclose($fp);
					}
					...
					CheckDirPath($_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/vsfr.merchant/logs/");
					if ($PROFILE_ID>0)
					{
						if ($ar_profile["IN_CRON"]=="Y")
						{
							// remove
							$cfg_data = preg_replace("#^.*?".preg_quote("/bitrix/php_interface/include/vsfr.merchant/")."cron_frame.php +".$PROFILE_ID." *>.*?$#im", "", $cfg_data);
						}
						else
						{
							if ($agent_period>0)
							...
							$cfg_data .= $strTime.$agent_php_path." -f ".$_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/vsfr.merchant/cron_frame.php ".$PROFILE_ID." >".$_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/vsfr.merchant/logs/".$PROFILE_ID.".txt\n";
						}
			...
			elseif ($_REQUEST["ACTION"]=="DEL_PROFILE" && $bCanEdit)
			{
				...
				if (strlen($strErrorMessage)<=0)
				{
					...
					CCatalogVSFRExport::Delete($PROFILE_ID);
				}
			}
			//////////////////////////////////////////////
			// EXPORT_SETUP
			...
			elseif (($_REQUEST["ACTION"]=="EXPORT_EDIT" || $_REQUEST['ACTION'] == 'EXPORT_COPY') && $bCanEdit)
			{
				...
				if ($boolFlag)
				{
					$arProfile = CCatalogVSFRExport::GetByID($PROFILE_ID);
					if ($arProfile)
					{
				...
				if ($boolFlag)
				{
					if (strlen($arReportsList[$arProfile['FILE_NAME']]["FILE_SETUP"])>0)
					{
						...
						{
							$NEW_PROFILE_ID = CCatalogVSFRExport::Update($PROFILE_ID,array(
								"NAME"			=> $SETUP_PROFILE_NAME,
								"SETUP_VARS"	=> $strSETUP_VARS,
								"NEED_EDIT"		=> "N",
							));
						...
						{
							$NEW_PROFILE_ID = CCatalogVSFRExport::Add(array(
								"LAST_USE"		=> false,
								"FILE_NAME"		=> $strActFileName,
								"NAME"			=> $SETUP_PROFILE_NAME,
								"DEFAULT_PROFILE" => "N",
...
foreach ($arReportsList as $strReportFile => $arReportParams)
{
	...
	$boolExist = false;
	$rsProfiles = CCatalogVSFRExport::GetList(
...
<form name="cronform_add" id="cronform_add" action="" method="post">
	...
	<tr>
		...
		<td><input type="text" name="agent_php_path" <?if(strlen(trim($_REQUEST["agent_php_path"]))>0):?>value="<?echo trim($_REQUEST["agent_php_path"]);?>"<?elseif(strlen($tp)<=0):?>value="/usr/local/php/bin/php"<?else:?>value="<?echo $tp;?>"<?endif;?> size="25"></td>
	</tr>
	<tr>
		<td style="font-size: 12px;"><? echo GetMessage("CES_AUTO_CRON"); ?></td>
		<td><input type="hidden" name="auto_cron_tasks" value="N"><input type="checkbox" name="auto_cron_tasks" value="Y" checked="checked"></td>
	</tr>
	<tr>
		<td colspan="2" style="text-align: center;">
...
echo BeginNote();
	echo GetMessage("export_setup_cat")?> /bitrix/php_interface/include/vsfr.merchant<br>
<br>
	...
	<?else:?>
		...
		echo GetMessage("CES_NOTES10");?><br>
<br>
		<?echo GetMessage("CES_NOTES11_EXT");?><br>
		<b><?echo $_SERVER["DOCUMENT_ROOT"];?>/bitrix/php_interface/include/vsfr.merchant/cron_frame.php</b><br>
		<?echo GetMessage("CES_NOTES12_EXT");?>
	<?endif;
