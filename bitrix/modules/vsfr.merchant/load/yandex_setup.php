...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/vsfr.merchant/export_setup_templ.php');
...
?><tr>
	...
	echo GetIBlockDropDownListEx(
		...
		"ClearSelected(); BX('id_ifr').src='/bitrix/tools/vsfr.merchant/vsfr_yandex_util.php?IBLOCK_ID=0&'+'".bitrix_sessid_get()."';",
		"ClearSelected(); BX('id_ifr').src='/bitrix/tools/vsfr.merchant/vsfr_yandex_util.php?IBLOCK_ID='+this[this.selectedIndex].value+'&'+'".bitrix_sessid_get()."';",
...
<tr>
	...
	<iframe src="/bitrix/tools/vsfr.merchant/vsfr_yandex_util.php?IBLOCK_ID=<?=intval($IBLOCK_ID)?>&<? echo bitrix_sessid_get(); ?>" id="id_ifr" name="ifr" style="display:none"></iframe>
	</td>
</tr>
		...
		function showDetailPopup()
		{
			if (!obDetailWindow)
			{
				...
				var obDetailWindow = new BX.CAdminDialog({
					'content_url': '/bitrix/tools/vsfr.merchant/vsfr_yandex_detail.php?lang=<?=LANGUAGE_ID?>&bxpublic=Y&IBLOCK_ID=' + s[s.selectedIndex].value,
...
<tr>
	...
	<td width="60%">
		...
		<input type="hidden" name="USE_HTTPS" value="Y"<? echo ($USE_HTTPS == 'Y' ? ' checked' : ''); ?>



		<input type="hidden" name="DISABLE_REFERERS" value="N">
		<input type="hidden" name="DISABLE_REFERERS" value="Y"<? echo ($disableReferers == 'Y' ? ' checked' : ''); ?>
...
<tr>
	...
	<td width="60%">
		<b><? echo htmlspecialcharsbx(COption::GetOptionString("catalog", "export_default_path", "/bitrix/catalog_export/"));?></b><input type="text" name="SETUP_FILE_NAME" value="<?echo (strlen($SETUP_FILE_NAME)>0) ? htmlspecialcharsbx($SETUP_FILE_NAME) : "merchant_".mt_rand(0, 999999).".php" ?>" size="50" />
	</td>
</tr>
<?
