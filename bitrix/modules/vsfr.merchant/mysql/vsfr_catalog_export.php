<?
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/vsfr.merchant/general/vsfr_catalog_export.php");

class CCatalogVSFRExport extends CCatalogVSFRExportALL
{
	function Add($arFields)
	{
		...
		if (!CCatalogVSFRExportALL::CheckFields("ADD", $arFields))
			return false;

		$arInsert = $DB->PrepareInsert("b_catalog_vsfr_export", $arFields);
		...
		$strSql =
			"INSERT INTO b_catalog_vsfr_export(".$arInsert[0].") ".
			"VALUES(".$arInsert[1].")";
	...
	function Update($ID, $arFields)
	{
		...
		if (!CCatalogVSFRExportALL::CheckFields("UPDATE", $arFields))
			return false;

		$strUpdate = $DB->PrepareUpdate("b_catalog_vsfr_export", $arFields);
		...
		$strSql =
			"UPDATE b_catalog_vsfr_export SET ".$strUpdate." WHERE ID = ".$ID." AND IS_EXPORT = 'Y'";
		$DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
