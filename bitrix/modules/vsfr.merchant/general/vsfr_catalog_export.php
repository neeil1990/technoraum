<?
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/vsfr.merchant/general/catalog_export.php");
class CCatalogVSFRExportALL extends CAllCatalogExport2
{
	...
	function Delete($ID)
	{
		...
		$ID = intval($ID);
		return $DB->Query("DELETE FROM b_catalog_vsfr_export WHERE ID = ".$ID." AND IS_EXPORT = 'Y'", true);
	}
	...
	function PreGenerateExport($profile_id)
	{
		...
		return "CCatalogVSFRExport::PreGenerateExport(".$profile_id.");";
	}
